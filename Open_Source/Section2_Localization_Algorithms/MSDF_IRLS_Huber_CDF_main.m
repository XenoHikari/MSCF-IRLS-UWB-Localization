...
COG = sum(Anchor_all,2) / numel(Anchor_all(1,:));

% Targets are chosen among 18m × 18m × 3m exclude deployment region
% for the CDF curves. Here the script evaluates multiple positions.
...
for seita_iter = 1:numel(Test_MS(1,:))
...
COG = sum(Anchor_all,2)./Anchor_all_Num;  %COG(Hotspot)
...
% and the subset-level weighted form in Eq. (55).
    ...
    for i = 2 : SS_Anchor_Num-1
        for j = i+1 : SS_Anchor_Num
            E(k,:) = e{i,j}';
            sigma_SS_matrix(k,k) = 1/(c^2*SS_delta_TOF_meas(j)^2*sigma_SS(j));
            k = k+1;
        end
    end
