...
%   including adaptive anchor subset planning, multi-subset consistency fusion,
%   and IRLS-Huber robust refinement.
%
...
% NLOS bias setting
% =========================
enable_NLOS = 1;  % 1:NLOS/0:LOS
p_NLOS = 0.2;
mu_NLOS = 1.0;

is_NLOS = false(1, Anchor_all_Num);
b_NLOS_range = zeros(1, Anchor_all_Num); 
b_NLOS_time = zeros(1, Anchor_all_Num);   

if enable_NLOS == 1 && mu_NLOS > 0
    candidate_idx = 1:Anchor_all_Num;  % All measurements could occur NLOS
    N_NLOS = round(p_NLOS * length(candidate_idx));

    if N_NLOS > 0
        rand_idx = randperm(length(candidate_idx), N_NLOS);
        nlos_idx = candidate_idx(rand_idx);

        is_NLOS(nlos_idx) = true;

        for k = 1:length(nlos_idx)
            i = nlos_idx(k);
            b_NLOS_range(i) = -mu_NLOS * log(max(rand(1), eps));
            b_NLOS_range(i) = min(b_NLOS_range(i), 3*mu_NLOS);
            b_NLOS_time(i) = b_NLOS_range(i) / c;
        end
    end
end
...
for i = 1:Anchor_all_Num
    ...
    TOF_meas(i) = TOF_real(i) + Noise(i) + b_NLOS_time(i);
end
